...
iRect iPopup::touchRect()
{
	...
	iRect rt;
	if (stat == iPopupOpen)
		rt.origin = linear(aniDt / _aniDt, openPoint, closePoint);
	else if (stat == iPopupProc)
		rt.origin = closePoint;
	else
		rt.origin = linear(aniDt / _aniDt, closePoint, openPoint);

	int minX = 10000, maxX = -10000, minY = 10000, maxY = -10000;
	for (int i = 0; i < arrayImg->count; i++)
	{
		iImage* img = (iImage*)arrayImg->objectAtIndex(i);
		iRect r = img->touchRect(rt.origin, iSizeMake(20, 20));
		if (minX > r.origin.x)
			minX = r.origin.x;
		if (maxX < r.origin.x + r.size.width)
	...
	return iRectMake(minX, minY, maxX - minX, maxY - minY);
}
