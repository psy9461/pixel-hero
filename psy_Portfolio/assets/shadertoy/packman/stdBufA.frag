...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    else if( state < 1.5 ) 
	{
        ...
        if( iKeyboard[1] > 0 ) pacmanMovDirNex.z = 1.0;
        if( iKeyboard[0] > 0 ) pacmanMovDirNex.z = 2.0;
        if( iKeyboard[2] > 0 ) pacmanMovDirNex.z = 3.0;
        if( iKeyboard[3] > 0 ) pacmanMovDirNex.z = 4.0;
