...
void startApp(HDC hdc, VOID_METHOD methodLoad)
{
    ...
    matrixProj->ortho(0, 500, 325, 0, -1, 1);
...
iPoint coordinate(int x, int y)
{
    ...
    if (devSize.width > viewport.size.width)
        rx = viewport.size.width / devSize.width;
    else
        rx = devSize.width / viewport.size.width;

    if (devSize.height > viewport.size.height)
        ry = viewport.size.height / devSize.height;
    else
        ry = devSize.height / viewport.size.height;

    //p.x = (x - viewport.origin.x) * rx;
    //p.y = (y - viewport.origin.y) * ry;

    p.x = (x - rtOpenGL.origin.x);// rx;
    p.y = (y - rtOpenGL.origin.y);// ry;
...
void clearWndListBox(HWND hwnd)
{
...
void setWndListBoxData(HWND hwnd, int index, char* str)
{
    ...
    wstr = utf8_to_utf16(str);
    SendMessage(hwnd, LB_SETITEMDATA, (WPARAM)index, (LPARAM)wstr);
}
...
void addEb(HWND hwnd, MethodEb method)
{
    if (ebl == NULL)
    {
        ebl = new EBList[_eblNum];
        eblNum = 0;
    }

    for (int i = 0; i < eblNum; i++)
    {
        EBList* l = &ebl[i];
        if (l->hwnd == hwnd)
        {
...
HWND createWndEditBox(const char* str, int x, int y, int width, int height, WndEditStyle style, MethodEb m)
{
    wchar_t* s = utf8_to_utf16(str);
...
HWND createWndEditBoxMultiline(const char* str, int x, int y, int width, int height, MethodEb method)
{
    wchar_t* s = utf8_to_utf16(str);
...
HWND createWndOpenGL(int x, int y, int width, int height, METHOD_OPENGL method)
{
    HWND hwndParent = hWnd;
    HINSTANCE instanceParent = (HINSTANCE)GetWindowLong(hwndParent, GWL_HINSTANCE);
    HWND hwnd = CreateWindow(WC_STATIC, NULL, WS_CHILD | WS_VISIBLE,
        x, y, width, height, hwndParent, (HMENU)0, instanceParent, NULL);
    HDC hdc = GetDC(hwnd);
    hDC = hdc;
    startApp(hdc, method);
    ...
    resizeApp(rt.right-rt.left, rt.bottom-rt.top);
    //mainLoop();
...
const char* openFileDlg(bool open, const char* filter)
{
    ...
    const char* path = openFileDlg(open, wFilter, L"0");
    delete wFilter;
